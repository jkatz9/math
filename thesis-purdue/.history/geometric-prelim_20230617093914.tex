...
\begin{align}
  \Riem_q (\frac{\partial}{\partial x^h}, \frac{\partial}{\partial x^i})\frac{\partial}{\partial x^j} = \sum_l \Riem^l_{jhi} \frac{\partial}{\partial x_l}.
\end{align}
for a collection of smooth function $\Riem^l_{jhi}$ of $x^1,...,x^d$. Then one has the following expressions for the Ricci curvature
\begin{align}
...
\begin{align}
  |\Riem|^2_q & = \sum_{i,j,k,l} (\Riem_{ijk}^l(q))^2 \\
  |\Ricci|^2  & = \sum_{i,j} (\Ricci_{ij})^2.
\end{align}
...
\begin{align}
  ...
  u_{M,2}(x)  & = \frac{1}{360} \left( 2|\Riem|^2_x -2 |\Ricci|^2_x +5 \scalar^2(x) \right) \label{eq:heattwo}
\end{align}
where $\scalar$, $\Ricci$, and $\Riem$ are the scalar, Ricci, and Riemannian curvature tensors.
