...
For each $p \in M$ and $x,y\in T_p M$, the \textbf{Ricci curvature} $\Ricc_p(x,y)$ is the trace of the endomorphism $\Riem_p(x,y) \in \End(T_p M)$. If $(e^1,\dots, e^d)$ is an orthonormal basis of $T_p M$ with respect to $g_p$, then 
\begin{align}
  \Ricci_p(x,y) = \sum_{j=1}^d g_p (\Riem_p(x,e^j)y,e)
\end{align} 
