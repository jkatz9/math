...
\begin{align}\label{eq:universalCover}
    \tilde{M} \to M
\end{align}
be its universal cover, and let $g^*$ be the metric on $\Mti$ pulled back along \ref{eq:universalCover}. Then $\pi_1(M)$ acts freely and properly discontinuously on $(\Mti,g^*)$ by isometries, transitively on the fibers of \ref{eq:universalCover}. Consequently, one has a

of dimension $2$ with constant scalar curvature $-1$, then there exists an isometry
\begin{align}\label{eq:uniformization}
    (\tilde{M},g^*) \to (\Hfrak , g_\hyp)
\end{align}
where $\tilde{M}$ is the universal cover and the metric $g^*$ is the pullback of $g$ through the universal covering map $\tilde{M}\to M$. The fundamental group $\pi_1(M)$ acts freely and cocompactly on $\tilde{M}$ by isometries with respect to $g^*$.
