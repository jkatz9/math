...
We view $A$ as four dimensional affine space over $F$, and write $F[A]$ for its coordinate ring. For $g \in A^\times$ and $f\in F[A]$, and write  $f^g$ for the function $x \mapsto  f( g x g^\inv)$. For $x\in A$, let $O(x)$ 
