...
\begin{proposition}
  ...
  \begin{description}
    \item[hyperbolic] if and only if $g$ is conjugate into the subgroup
      ...
      if and only if $g$ preserves complete geodesic $\gamma$ on which it restricts to an action if and only if $\tr(g)^2 -4 >0$ if and only if $g$ is regular semisimple and split over $\Rbb$
