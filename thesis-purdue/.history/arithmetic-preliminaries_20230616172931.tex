...
\begin{definition}
    A subgroup $\Lambda$ of $\SL(2,\Rbb)$ (resp. $\SL(2,\Cbb)$) is \textbf{arithmetic} if there exists a quaternion algebra $A$ over a number field $k$ of Fuchsian (resp. Kleinian) type and an order $O$ in $A$ such that $\Lambda$ is commensurable (in the wide sense) with the arithmetic lattice $\Gamma_{O}$ associated to $O$. We say that $\Lambda$ is \textbf{derived from} $O$ if it is conjugate in $\SL(2,\Rbb)$ (resp. $\SL(2,\Cbb)$) to a subgroup of $\Gamma_O$. Say that $\Lambda$ is \textbf{derived from} $A$ if it is derived from some order in $A$. If $M$ is a hyperbolic $2$- or $3$- manifold, we say that $M$ is arithmetic (resp. derived from an order $O$ or a quaternion algebra $A$) if there exists an arithmetic lattice $\Lambda$  such that $M$ is isometric to $\Lambda \lmod \Hbb$ for some arithmetic lattfice $\Lambda$ (resp. arithmetic lattice derived from $O$ or $A$).
\end{definition}
