...
Now suppose $g$ is a Riemannian metric on $M$, For a vector $v$ define a $1$-form $v^*$ by the rule $v^*(x)=g(v,x)$ for all vectors $x$, and for a $1$-form $\alpha$, define a vector $\alpha_*$