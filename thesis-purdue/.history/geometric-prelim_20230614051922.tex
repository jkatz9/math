...
Let $(M,g)$ be a compact Riemannian manifold, $\tilde{M} \to M$ its universal cover, and $\Gamma =\pi_1(M)$ its fundamental group. There is a unique metric $g^*$ on $\Mti$ such that the action of $\Gamma$ by deck transformations is isometric, and such that   on   Then $\pi_1(M)$ acts freely and properly discontinuously on $(\Mti,g^*)$ by isometries, transitively on the fibers of \ref{eq:universalCover}.
