...
\end{align}
Then $L^2(\Gamma \lmod G , \dop \mu_*)$ may be identified with the Hilbert space $\Hcal$ of  left-$\Gamma$ invariant functions $\phi$ on $G$ for which $||\phi||^2:=\int_\Gamma \lmod G |\phi|^2 \dop \mu_* <\infty$. For $\phi \in L^2(X)$ and $g \in G$, set $(U(g)f) (x) = f(xg)
