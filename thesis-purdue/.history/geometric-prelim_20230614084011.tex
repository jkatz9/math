...
\end{align}
Then $L^2(\Gamma \lmod G , \dop \mu_*)$ may be identified with the Hilbert space $\Hcal$ of  left-$\Gamma$ invariant functions $\phi$ on $G$ for which $||\phi||^2:=\int_\Gamma \lmod G |\phi|^2 \dop \mu_* <\infty$. For $\phi \in L^2(X)$ and $y \in G$, set $(U_\Gamma(y)\phi) (x) = \phi(xy)$. The unitary representation $U_\Gamma:G \to \Aut(\Hcal)$ is completely reducible: writing $\Irr(G)$ for the set of equivalence classes of unitary representations of $G$, and letting $m_\Gamma : \Irr G \to \Zbb_{\geq 0}$ be the multiplicity of $m_\Gamma(\pi)  = \dim \Hom_G ()$  $m(\pi, U)$ denotes the multiplicity of an irreducible representation $\pi$ of $G$
