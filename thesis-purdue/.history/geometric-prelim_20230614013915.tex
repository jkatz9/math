...
Let $(M,g)$ be a compact Riemannian manifold with negative scalar curvature. The set of lengths of closed geodesics on $M$ is a discrete subset of $\Rbb_{>0}$ and for each length $\ell$, the number $m_{M,g}(\ell)$ of geodesics on $M$ with length $\ell$ is finite. We let $\Lspec_{M,g}$ denote the discrete measure on $\Rbb$ 
