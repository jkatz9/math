...
Let $D$ be an unbounded, self-adjoint operator acting on a hilbert space $\Hcal$ and suppose that the spectrum of $D$ is a discrete set of nonnegative real numbers, each occuring with finite multiplicity. We write $\spec D$ for the sequence $\lambda_0 \leq \lambda_1 \leq \dots$ of eigenvalues, repeated according to their multiplicity. Then there exists an orthonormal basis $\phi_k$ of $\Hcal$ satisfying $D \phi_k = \lambda_k \phi_k$. Given a function $f: \Rbb_{\geq 0} \to \Cbb$ such that the sum $\sum_{k\geq 0}f(\lambda_k)$ converges, we may define an operator $f(D)$ on $\Hcal$ by the rule
\begin{align}
    f(D) v = \sum_{k\geq 0} f(\lambda_k) \ip {v}{\phi_k} \phi_k
\end{align}
for any $v\in \Hcal$.

Now suppose furthermore that $\Hcal = L^2(X,\dop \mu)$ for some measure space $(X,\dop \mu)$. Then the operator $f(D)$ admits an integral kernel $K_f : X\times X \to \Cbb$, so that
\begin{align}
    f(D) v(x) = \int_M K_f(x,y)v(x) \dop \mu(y).
\end{align}
