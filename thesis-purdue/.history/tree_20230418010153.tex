...
The Bruhat-Tits tree $\Tcal$ for $\GL(2,k)$ is the following graph: the vertices of $\Tcal$ are homothety classes of $R$-lattices in $k^2$. Two vertices $\Lambda_1,\Lambda_2 $ of $\Tcal$ are adjacent if there are representative lattices $L_i \in \Lambda_i$ such that $\pi L_1 < L_2 < L_1$ with each inclusion proper.
...
\end{definition}
 ...
 We regard $k^2$ as a space of column vectors, with distinguished basis vectors $e_1,e_2$, so that $\GL(2,k)$ acts from the left.  We write $L_0 = Re_1+Re_2$ and refer to this as our distinguished lattice, 
