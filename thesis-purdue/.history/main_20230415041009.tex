...
\begin{claim}
	\begin{enumerate}
		\item The algebra $F[A]^{A^\times}$ (resp. $F[A^\times]^{A^\times}$) of invariants in $F[A]$ (resp. $F[A^\times]$) under $A^\times$ is generated as an $F$ algebra by $\trd$ and $\nrd$ (resp. $\trd,\nrd^{\pm 1}$).  
		\item The restriction of the map $x\mapsto (\trd(x),\nrd(x))$  to the open subvariety $A^\circ = \{x\in A^\times: \delta(x) \neq 0\}$ of $A^\times$ is a categorical quotient map for the conjugation action of $A^\times$ on $A$ (resp. on $A^\times$) in the sense that f
	\end{enumerate}
\end{claim}

Let $A^\circ$ denote the principal open subvariety of $A^\times$ consisting of elements $x$ for which $\delta(x)\neq 0$.   
