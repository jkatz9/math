...
\begin{facts}
    ...
    \begin{enumerate}
        \item If $O,O'$ are two orders in a quaternion algebra $A$, then $\Gamma_O$ and $\Gamma_{O'}$ are commensurable in $A^\times$. Consequently, the property of being commensurable in $G_\infty$ to an order in $A$ depends only on $A$. if $
        \item
    \end{enumerate}
\end{facts}
