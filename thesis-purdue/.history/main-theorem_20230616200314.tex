...
\begin{proof}
  ...
  \end{align}
where $\scalar$, $\Ricci$, and $R$ are the scalar, Ricci, and Riemannian curvature tensors, and where $
  ...
  \end{align}
  depends only on the spectrum of $M$. Consequently, integrating over $M$ in \ref{eq:heatAsymptoticM} and letting $a_{M,k} = \int_{M} u_{M,k}(x) \dop \Vol_M(x)$  we have
  \begin{align}
    \sum_{k\geq 0 }e^{-t \lambda_k} = \frac{1}{(4\pi t)^{\dim M /2 }} \sum_{k=0}^\infty a_{M,k}t^k, \quad \text{ as $t\to 0^+$.}
  \end{align}
