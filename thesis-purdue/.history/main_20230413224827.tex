...
\subsection*{Split case}
 In this section, let $G=\SL(2,k)$ and define closed subgroups 
\begin{itemize}
	\item $A = \{ a(x)=\tbt{x}{}{}{x^\inv}: x \in k^\times \}$
	\item $U = \{ u(y)=\tbt{1}{}{y}{1}: y \in k \}$
	\item $B=AU$
\end{itemize}
For a positive integer $n$, let $\pi_n : \SL(2,R) \to \SL(2,R/\mfrak^n)$ be the natural projection. For any subgroup $H$ of $G$, we set $H(R) = H \cap \SL(2,R)$, and $H(\mcal^n)= \ker \pi_n\vert_H$.  
