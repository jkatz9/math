...
By class field theory, the set $\Ram A$ of places over which $A$ ramifies is a finite set with even cardinality. Conversely, given any finite set $S \subset V_k$ of even cardinality and such that $S\cap V_k^{\infty,\Cbb}=\emptyset$ there is a unique quaternion algebra $A$ with $\Ram A = S$, up to isomorphism. $A$ is 
...
\begin{proposition}
    Let $A$ be a quaternion algebra over a number field $k$, and $O$ an order in $A$. If $A$ is of Fuchsian (resp. Kleinian) type , then $\Gamma_O$ is a lattice in $G_\Rbb  \approx \SL(2,\Rbb)$ (reps. in $G_\Cbb\approx \SL(2,\Rbb)$). In either case, $\Gamma_O$ is a cocompact lattice if and only if $A$ is a division algebra.
\end{proposition}
