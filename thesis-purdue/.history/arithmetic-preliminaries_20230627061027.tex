...
Then one has the containments $A^1 \leq  \ul{A}^1 \leq \ul{A}^{(1)}$.
...
% Up to isomorphism, $ M(2,k)$ is the unique unramified quaternion algebra over $k$.  In this case, the reduced trace and norm of an element $\alpha$ coincide with the trace and determinant maps for the action of $\alpha$ as an endomorphism of $k^2$.  The subring $M(2,R)$ is a maximal order in $M(2,k)$, and any maximal order is conjugate to $M(2,R)$ by an element of $\GL(2,k)$.

Let $G = \SL(2,k)$,  $K = \SL(2,R)$, and for $n>0$ let $K(n)$ be the kernel of the reduction map  $K \to  \SL(2,R/\pfrak^n)$.
