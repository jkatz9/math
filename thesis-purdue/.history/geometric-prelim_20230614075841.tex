...
Suppose now that $G$ is a semisimple Lie group, $K$ is a maximal compact subgroup of $G$, and that $\Gamma$ is a  cocompact lattice in $G$. The killing form on $\gfrak$ induces a Riemannian metric $g^*$ on $G/K$, which pushes out to a metric $g$  on $\Gamma \lmod G \rmod K$. In this case, the measure $\dop \vol_{g^*}$ is a Haar measure on $G$
