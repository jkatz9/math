...
We say that two classes $\Lambda,\Lambda'$ are adjacent, and write $\Lambda\sim\Lambda'$ if $d(\Lambda,\Lambda')=1$. The resulting graph $(\Hcal,\sim)$ is model of for the \textbf{Bruhat-Tits} tree $\Tcal$ for $\SL(2,k)$.

As an alternative model for the Bruhat-Tits tree $\Tcal$, we can equip the set $\Mcal$ of maximal orders with the adjacency relation $\Ocal \sim \Ocal'$ provided $|\Ocal: \Ocal\cap \Ocal'| = |R:\mfrak| = q$. 
