...
\begin{proof}
    ...
    \end{align*}
    we find that $xc \equiv 0 \mod \pfrak$, so that either $x$ or $c$ is in $\pfrak$. If $x\in \pfrak$, then $g \in O^1(\pfrak^n)$ already. If this is so for all $g \in \Lambda$, then the claim is proven. Otherwise, suppose $g$ has $x \in R_\pfrak^\times$ so that $c \in \pfrak$. Then, with $\alpha_o = \Tbt{\varpi}{0}{0}{1}$, we find that
    \begin{align*}
        \alpha_o g \alpha_o^\inv & = 1 + \varpi^{n-1}(\tbt{0}{\varpi  x}{0}{0} + \varpi \tbt{a}{\varpi b}{\varpi^\inv c}{d})
    \end{align*}
