...
\begin{claim}\label{orbits}
	For noncentral $x\in A^\times$, the orbit $O(x)$ is Zariski-closed in $A$ if and only if $x$ is not parabolic. All parabolic elements in $A^\times$ constitute a single conjugacy class, say $\Ucal$. The complement of $\Ucal$ in its Zariski closure $\overline{\Ucal}$ is the center $F^\times$ of $A^\times$.   
\end{claim}
\begin{claim}
