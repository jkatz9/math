...
\begin{proof}
    ...
    Now suppose that $\tr g \equiv 2 \mod \pfrak^{2n}$ for every $g\in \Lambda$. By inductive hypothesis, after conjugating by an element of $A^\times$ if necessary, we may assume $\Lambda \leq O^1(\pfrak^{n-1})$. Furthermore, the map $1 + \varpi^{n-1} X \mapsto X \mod \pfrak $ induces an isomorphism $O^1(\pfrak^{n-1})/O^1(\pfrak^n) \approx \sl(2,\kfrak_\pfrak)$
