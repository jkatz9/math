...
\begin{claim}	
	If $x\in A^\times$ is central, then $F[x] = F$. Otherwise, for noncentral $x$, with $\delta = \delta(x)$,
	\begin{align*}
...
\begin{claim}\label{orbits}
	For noncentral $x\in A^\times$, the orbit $O(x)$ is Zariski-closed in $A$ if and only if $x$ is not parabolic. All parabolic elements in $A^\times$ are conjugate, 
\end{claim}
\begin{claim}
