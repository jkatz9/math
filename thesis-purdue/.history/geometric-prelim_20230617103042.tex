...
By a theorem of Minakshisundaram and Pleijel \cite{Minakshisundaram.Pleijel-[PropertiesEigenfunctionsLaplaceoperator]1949} there exist a sequence of functions $u_{M,g}^{} : M \to \Rbb$ such that for each $x \in M$, the value $u_{M,g}^{(k)}(x)$ is given by universal formulae in terms of the curvature tensor of $M$ and its covariant derivatives at $x$ such that
...
\end{align}
where $\scalar$, $\Ricci$, and $\Riem$ are the scalar, Ricci, and Riemannian curvature tensors. As the heat kernel $K_{M,g}(x,y,t)$ depends only on the the Riemannian structure $(M,g)$, the numbers
\begin{align}
  a_{M,g,k} = \int_M u_{M,g}
\end{align}
