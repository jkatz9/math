...
We view $A$ as four dimensional affine space over $F$, and write $F[A]$ for its coordinate ring. For $g \in A^\times$ and $f\in F[A]$, and write  $f^g$ for the function $x \mapsto  f( g x g^\inv)$. For $x\in A$, write $O(x) = \{ x^g : g \in A^\times\}\subset A$ for its orbit under conjugation by $A^\times$. 

\begin{claim}\label{orbits}
	For $x\in A$, the orbit $O(x)$ is Zariski-closed in $A$ if and only if $x$ is not parabolic.
\end{claim}
\begin{claim}
