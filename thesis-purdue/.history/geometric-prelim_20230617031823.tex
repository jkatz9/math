...
Let $M$ be a smooth manifold. A \textbf{linear connection} on $M$ is an $\Rbb$-bilinear pairing $(X,Y) \mapsto D_X Y$ from $\Gamma(TM)\times \Gamma(TM)
...
\begin{description}
  ...
  \item[curvature] $R(X,Y) Z = D_Y D_X Z - D_X D_Y Z + D_{[X,Y]}Z$ for all $X,Y,Z \in \Xcal(M)$
\end{description}
