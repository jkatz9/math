...
\begin{proof}
  ...
  \end{align}
  we find that the quantities $\dim M$ and $\int_M u_{M,k}(x) \dop \vol_M(x)$ depend only on the spectrum of $M$. Carying out the same computations for $X(\Gamma_0(\afrak))$ we find that $\dim M = \dim X(\Gamma_0(\afrak))$ and from \ref{eq:heatzero} we have $\vol M = \vol X(\Gamma_0(\afrak))$.
  % \begin{align}
