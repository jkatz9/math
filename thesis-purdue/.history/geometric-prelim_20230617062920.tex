...
\begin{align*}
  ...
  \end{array}
\end{align*}
says that for each $p \in M$, the endomorphism $R_p (u_p,v_p) \in \gl(T_pM)$ lies in $\so(T_p M , \ip{}{}_p )$. The property $R(u, v)=-R(v, u)$ says that $R_p: T_pM \times T_pM \to \gl T_pM$ is alternating, so factors thru $T_pM \ 
