...
Let $R$ be a commutative ring (with $1$). An algebra $(A,\iota)$ over $R$ is an associative ring $A$ with $1$ equipped with an embedding $\iota : R \to A$ of rings, mapping $1 \in R$ to $1 \in A$ such that $\iota(R)$ is contained in the center of $A$. Using $\iota$, we identify $R$ with a central subring of $A$ and suppress reference to $\iota$.

A \textbf{standard involution} on $B$ is an anti-involution $\overline{\cdot}: A \to A$ such that $x\overline{x} \in A$ for all $x\in A$. 
\begin{defn}
