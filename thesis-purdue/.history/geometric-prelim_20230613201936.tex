...
\begin{facts}
    ...
    \begin{enumerate}
        Let $\Kbb = \Rbb$ or $\Cbb$.
        \item\label{item:orders} If $O,O'$ are two orders in a quaternion algebra $A$, then $\Gamma_O$ and $\Gamma_{O'}$ are commensurable in $A^1$. Consequently, the property of being commensurable in $G_$ to an order in $A$ depends only on $A$.
