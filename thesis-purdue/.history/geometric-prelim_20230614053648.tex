...
We identify functions on $M$ with $\Gamma$ invariant functions on $\Mti$.  For a smooth compactly supported function $f \in C_c^\infty(\Mti)$, the function $P_\Gamma(f) : x \mapsto \sum_{\gamma \in \Gamma}f(\gamma x)$ descends to a smooth function on the quotient $\Gamma \lmod \Mti \approx M$. The operator $P_\Gamma:C_c^\infty$
