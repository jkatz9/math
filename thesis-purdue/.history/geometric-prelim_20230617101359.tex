...
\end{proposition}
\begin{proof}
  Let let $\{\lambda_k : k\geq 0\}$ be the common sequence $0 = \lambda_0 < \lambda_1 \leq \lambda_2 \leq \dots $ of Laplace eigenvalues for $(M,g)$ and $(M',g')$ and pick an orthonormal sequence of eigenfunctions $\phi_k$ on $M$ (resp. $\phi_k'$ on $M'$) with eigenvalues $\lambda_k$. Then one can express the heat kernels for $(M,g')
\end{proof}
