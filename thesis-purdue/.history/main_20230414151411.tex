...
\begin{claim}	
	If $x\in A$ is central, then $F[x] = F$. Otherwise, for noncentral $x\in A$:
	\begin{align*}
		F[x] \approx \begin{cases}	
						F \oplus F 			& \text{if $\delta(x) \in  (F^\times)^2$ } \\
						F[T]/(T^2 - \delta(x)) & \text{if $\delta(x) \in  F^\times \setminus (F^\times)^2$} \\
						F[T]/(T^2 ) & \text{if $\delta(x)=0$ },
					\end{cases}
	\end{align*}
	and we say $x$ is split semisimple, nonsplit semisimple, or parabolic respectively. 
	When $x$ is noncentral, its centralizer in $A^\times$ is $F[x]^\times$.
\end{claim}


The group $A^\times$ acts on $A$ by conjugation: 
\begin{align*}
	\Int(g)x = gxg^\inv,
\end{align*}
and this action preserves the reduced norm and trace maps. 

We view $A$ as four dimensional affine space over $F$, and write $F[A]$ for its coordinate ring. For $g \in A^\times$ and $f\in F[A]$, and write  $f^g$ for the function $x \mapsto  f(\Int(g) x)$. 

\begin{claim}
	The functions $\trd(x), \nrd(x)$ span the algebra $F[A]^{A^\times}$ of invariants under $A^\times$. x
\end{claim}
