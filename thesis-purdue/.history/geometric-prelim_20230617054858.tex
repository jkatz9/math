...
For a surface $M$ embedded in $\Rbb^3$, in a nbd of $x\in M$, pick a unit normal vector $n=n(y)$. Then $n: U_x \to S^2 \subset \Rbb^3$ where $u_x$ is a nbd of $x$. Then $T_xM$ and $T_{n(x)} S^2$ are parallel planes and $\det d n (x)$ is well defined, and is the gaussian curvautre. 
