...
\end{proposition}
\begin{proof}
  At each $p \in M$ the Ricci curvature is a symmetric operator on $T_p M$ relative to the bilinear form $g_p$. Consequently, at each $p$, one can pick a basis $e^1,\dots,e^d$ of $T_p M$ for which $\Ricci_p = _{ii}(p)e^i e^i$ is diagonal. Then one has  
  \begin{align}
