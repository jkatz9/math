...
\begin{definition}
\end{definition}
A subgroup $\Lambda$ of $\SL(2,\Rbb)$ (resp. $\SL(2,\Cbb)$) is \textbf{arithmetic} if there exists a quaternion algebra $A$ over a number field $k$ of Fuchsian (resp. Kleinian) type and an order $O$ in $A$ such that $\Lambda$ is commensurable (in the wide sense) with the arithmetic lattice $\Gamma_{O}$ associated to $O$. We say that $\Lambda$ is \textbf{derived} from the order $O$ if it is conjugate in $\SL(2,\Rbb)$ (resp. $\SL(2,\Cbb)$) to a  
\end{definition}
