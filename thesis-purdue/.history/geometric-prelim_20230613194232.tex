...
\begin{proposition}
    Let $A$ be a quaternion algebra over a number field $k$, and $O$ an order in $A$. If $A$ is of Fuchsian (resp. Kleinian) type , then $\Gamma_O$ is a lattice in $G_\Rbb  \approx \SL(2,\Rbb)$ (reps. in $_\Cbb)
\end{proposition}
