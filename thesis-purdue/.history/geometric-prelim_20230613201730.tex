    ...
    \begin{enumerate}
        ...
        \item If $O,O'$ are orders in quaternion algebras $A,A'$ over number fields $k,k'$ with
              \begin{align*}
                  A_\infty :=\prod_{\nu \in V_k^\infty}A_\nu \approx \prod_{\nu \in V_{k'}^\infty}A'_\nu =: A'_\infty
              \end{align*}
              such that $\Gamma_O$ is commensurable to $\Gamma_O'$ in $A_\infty^1 = A'_\infty^1$ then $k=k'$ and $A = A'$. Combining this fact with \ref{item:orders}, it follows that the quaternion algebra associated to an arithmetic Fuchsian or Kleinian lattice
    \end{enumerate}
\end{facts}
