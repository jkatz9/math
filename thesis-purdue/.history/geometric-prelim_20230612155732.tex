...
\begin{enumerate}
    \item For each fixed $y\in M$, one has $\left(\frac{\partial}{\partial t} + \Delta \right)k(t,x,y) = 0$
    \item For each $u\in C^\infty(M)$, one has $ \int_M k(t,x,y) u(y)  \dop \Vol_g (x)(y) \to u(x)$  as $t \to 0^+$, uniformly in $x$
    \item $k(t,\cdot,\cdot)$ is $C^2$ for each $t$
    \item $k(\cdot, x,y)$ is $C^1$ for each $(x,y)$.
\end{enumerate}
In particular, the heat kernel $K(t,x,y)$ is independent of the choice of orthonormal basis of eigenfunctions in the defining equation \ref{eq:heatdef}.
