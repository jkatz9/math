...
\end{enumerate}
We extend $s$ to $\kti^\times$ by $s(0)=0$ and write $s_r = \eta_r \circ s : \kti \to \Rti_r$. Then each $x \in \Rti$ admits a unique representation as $x = \sum_{i\geq 0} s(x_i)\varpi^i$ for some $x_i \in \kti$. Likewise, each $x \in \Rti_r$ admits a unique representation $x = \sum_{i=0}^{r-1} s(x_i)(\pi +\pi^r \Rti)^i$.
