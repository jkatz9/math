...
In this section $F$ is a number field with number field $R$, $A$ a quaternion algebra over $F$, and $\Ocal$ a maximal order in $A$. The algebra $A$ is equipped with an (anti-)involution $\overline{\cdot}$ and reduced norm/trace maps: 
\begin{align*}
	\trd(x) &= x+\overline{x} \\
	\nrd(x) &= x\overline{x},
\end{align*} 
...
\begin{align*}
	\Int(g)x = gxg^\inv,
\end{align*}
and this action preserves the reduced norm and trace maps. Recall that the characterstic polynomial $\chi(x;T)$ has repeated roots if and only if its discriminant $\delta(x) = \trd(x)^2 - 4\nrd(x)$ is zero. We may view $A$ as four dimensional affine space over $F$, and the functions $\trd$, $\nrd$ as regular maps $A \to \Abb^1_F$ (the affine line over $F$). 
