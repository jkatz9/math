...
\end{align*}
\hrule
Let $p \in M$ and $U_p \subset M$ be a neighborhood on which geodesic normal coordinates can be defined $(x_1,...,x_d) :  B \to U_p$ where $B$ is a ball about $0$ in $\Rbb^d$. In this coordinate system, $g_{ij} = \delta_{ij} + O(|x|^2)$ and 
\begin{align*}
  g_{ij} = \delta_{ij} - \sum_{k}\frac{1}{3} R_{ikjl}x^kx^l + O(|x|^3)
\end{align*}
\hrule
