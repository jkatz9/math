...
A \textbf{Koszul connection} on a smooth manifold $M$ is a function $(X,Y) \mapsto \nabla_X Y$ from $\Gamma (TM) \otimes \Gamma(TM) \to \Gamma(TM)$ which is $C^\infty(M)$ linear in $X$, while in $Y$:
\begin{align*}
  \nabla_X (f Y) = f \cdot \nabla_X Y + X(f)\cdot Y
\end{align*}
for every $f\in C^\infty(M)$ and $X,Y \in \Gamma(TM)$. For such a connection, one has $\nabla_{X_p} Y \in T_p M$ for any $p \in M$ and $Y \in \Gamma(TM)$.

Let $x^1, \dots,x^n$ be a local coordinate system on $M$, and define $\Gamma_{ij}^k$ by the formula:
\begin{align*}
  \nabla_{\partial x^i }\partial x^j = \sum_{k=1}^n \Gamma_{ij}^k \partial x^k
\end{align*}

Given $Y\in \Gamma (TM)$, then $\nabla Y \in \end \Gamma (\End TM)$. 
\newpage
