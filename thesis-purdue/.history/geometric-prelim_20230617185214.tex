...
\begin{proposition}
  ...
  \begin{description}
    ...
    \item[elliptic] if and only if $g$ is conjugate into the subgroup
      ...
      if and only if $g$ fixes a unique point in $\Hfrak$ if and only if $\tr(g)-4 < 0$ if and only if $g$ is regular and semisimple but not split over $\Rbb$
    \item[parabolic] if and only if $g$ is conjugate into the subgroup
      \[P =\{\Tbt{1}{x}{0}{1}: x \in \Rbb \}\]
      if and only if $g$ fixes a unique end of $\Hfrak$ if and only if $\tr g
  \end{description}
\end{proposition}
