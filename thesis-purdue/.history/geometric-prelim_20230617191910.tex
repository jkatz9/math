...
\\{\Tbt{a}{0}{0}{a^\inv}: a \in \Rbb \}\]subsection{some basic hyperbolic geometry}
...
If $g$ is hyperbolic, then modulo $\pm 1$, it is conjugate to a unique matrix  of the form $\Tbt{\lambda_g}{}{}{\lambda_g^\inv }$ where $\lambda_g \in \Rbb_{>0}$ and $\lambda_g > \lambda_g^\inv$. In this case, the translation length $\tau(g)$ is related to $\lambda_g$ by
\begin{align}
  \tau(g) = 2 \log \lambda_g,
\end{align}
which in turn, can be expressed in terms of $\tr(g) = \lambda_g + \lambda_g^\inv$. Indeed, $\{\lambda_g^{\pm 1}\}$ are the roots of the characterstic polynomial $x^2 - \tr g x +1$  so that $\lambda_g^{\pm 1} = \frac{\tr g \pm \sqrt{\tr (g)^2 -4}}{2}$ so that
\begin{align}
  |\tr g|  = e^{ \tau(g)/2} + e^{-\tau(g)/2} = 2 \cosh \tau(g) /2.
\end{align}



If $\Lambda $ is a cocompact subgroup of $\SL(2,\Rbb)$ which is torsion free modulo $\pm 1$, and consists of purely hyperbolic elements, then the quotient $\Lambda \lmod \Hfrak$ is a compact hyperbolic surface. The following proposition asserts a converse:

\begin{proposition}[Uniformization]
  Let $(M,g)$ be a smooth compact surface with constant scalar curvature $-1$. Then there exists a cocompact lattice $\Lambda < \SL(2,\Rbb)$ which is torsion free modulo $\pm 1$, and consists only of hyperbolic elements such that $(M,g)$ is isometric to $\Lambda \lmod \Hfrak$. Up to conjugation in $\SL(2,\Rbb)$ the lattice $\Lambda$ is uniquely determined by the Riemannian metric on $M$.
\end{proposition}
