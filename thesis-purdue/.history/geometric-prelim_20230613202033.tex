...
\begin{facts}
    ...
    \begin{enumerate}
        ...
        \item If $O,O'$ are orders in quaternion algebras $A,A'$ over number fields $k,k'$ with $A_\Kbb = A'_\Kbb$
              such that $\Gamma_O$ is commensurable to $\Gamma_O'$ in $AA_\Kbb^1 = A'_\infty^1$ then $k=k'$ and $A = A'$. Combining this fact with \ref{item:orders}, it follows that the quaternion algebra associated to an arithmetic Fuchsian or Kleinian lattice is uniquely
    \end{enumerate}
\end{facts}
