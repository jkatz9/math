...
Let $(M,g)$ be a compact Riemannian manifold, $\tilde{M}$ its universal cover, and $\Gamma =\pi_1(M)$ its fundamental group. There is a unique metric $g^*$ such that the covering map $(\Mti, g^*) \to (M,g)$ is a local isometry.   on   Then $\pi_1(M)$ acts freely and properly discontinuously on $(\Mti,g^*)$ by isometries, transitively on the fibers of \ref{eq:universalCover}.
