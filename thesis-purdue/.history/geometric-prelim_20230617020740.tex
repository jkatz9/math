...
\begin{proof}
  ...
  \end{align}
  with equality if and only if $\scalar_{g'}$ and $1$ are linearly dependent as functions on $M'$, which is to say: if and only if $\scalar_{g'}$ is constant. Now note that 
\begin{align}
  \int_{M'}\scalar_{g'}\dop v_{g'} &=\int_{M}\scalar_{g}\dop v_{g},
  \int_{M'}\scalar_{g'}^2\dop v_{g'} &=\int_{M}\scalar_{g}\dop v_{g}
\end{align}
\end{proof}
\newpage
