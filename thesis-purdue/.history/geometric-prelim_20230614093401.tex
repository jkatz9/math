...
\begin{align}
    ...
    %   & = \int _{\Gamma \lmod G}\sum_{c \subset \Gamma} \sum_{\gamma \in c } f(x^\inv \gamma x ) \dop \mu_*(x) \\
                                                  ...
                                                  & = \sum_{\gamma \in C_\Gamma} \Vol(\Gamma_\gamma \lmod G_\gamma) I_\gamma(f).
\end{align}
where $G_\gamma$ is the centralizer of $\gamma$ in $G$, and $I_\gamma(f)$ is the \textbf{orbital integral} $I_\gamma(f) = \int_{G_\gamma \lmod G} f(x^\inv \gamma x)
