...
For an order  $\Ocal \subset A$ in $A$, write $\Ocal^1 = A^1 \cap \Ocal$. We call the subgroup $\psi(\Ocal^1)\leq \PSL(2,\Rbb)$ \textbf{the arithmetic Fuchsian group associated to the datum} $k$,$A$,$\psi$,$\Ocal$. 
...
Let $V$ be a $2$-dimensional vectorspace over $k$. Then $\End_k(V)$ is a quaternion algebra over $k$. The following proposition identifies the maximal orders in $\End_k(V)$:
\thm
