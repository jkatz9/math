...
Let $M$ be a smooth manifold, and write $\Xcal(M) = \Gamma(TM)$ for the $C^\infty(M)$ module of vectorfields on $M$. A $p$-multilinear form on $\Xcal(M)$ is an element of $\Gamma(\bigotimes^p T^*M)$ if and only if it is $p$-multilinear over $C^\infty(M)$. For $X,Y\in \Xcal(M)$, the map $(X,Y) \to D_
