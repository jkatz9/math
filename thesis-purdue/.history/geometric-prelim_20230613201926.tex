...
\begin{facts}
    ...
    \begin{enumerate}
        Let $\Kbb = \Rbb$ or $\Cbb$.
        \item\label{item:orders} If $O,O'$ are two orders in a quaternion algebra $A$, then $\Gamma_O$ and $\Gamma_{O'}$ are commensurable in $A^\times$. Consequently, the property of being commensurable in $G_\infty$ to an order in $A$ depends only on $A$.
        \item If $O,O'$ are orders in quaternion algebras $A,A'$ over number fields $k,k'$ with $A_\Kbb = A'_\Kbf$
              such that $\Gamma_O$ is commensurable to $\Gamma_O'$ in $A_\infty^1 = A'_\infty^1$ then $k=k'$ and $A = A'$. Combining this fact with \ref{item:orders}, it follows that the quaternion algebra associated to an arithmetic Fuchsian or Kleinian lattice is uniquely
    \end{enumerate}
\end{facts}
