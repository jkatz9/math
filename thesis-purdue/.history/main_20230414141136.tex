...
We view $A$ as four dimensional affine space over $F$, and write $F[A]$ for its coordinate ring. For $g \in A^\times$ and $f\in F[A]$, and set $f^g(x)= f(\Int(g) x)$  The functions $\trd$, $\nrd$ are regular map
