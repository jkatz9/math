...
The Bruhat-Tits tree $\Tcal$ for $\GL(2,k)$ is the following graph: the vertices of $\Tcal$ are homothety classes of $R$-lattices in $k^2$, where $k^2$ is regarded as a space of column vectors. Two vertices $\Lambda_1,\Lambda_2 \in V(\Tcal)$ are adjacent if there are representative lattices $L_i \in \Lambda_i$ such that $\pi L_1 < L_2 < L_1$ with each inclusion proper.
\end{definition}
