...
\begin{proof}
	...
	Now suppose that $\tr g \equiv 2 \mod \pfrak^{2n}$ for every $g\in \Lambda$. By inductive hypothesis, after conjugating if necessary, we may assume $\Lambda \leq O^1(\pfrak^{n-1})$. Thus, each element $g\in \Lambda$ can be written as
	\begin{align*}\label{eq:AppleWensleydale}
		g = \id +\varpi^{n-1} \gamma
	\end{align*}
	for some $\gamma \in M(2, R_\pfrak)$. Note that since $\tr g \equiv 2 \mod \pfrak^{2n}$, one has $\tr(\gamma) \equiv 0 \mod \pfrak^{n+1}$. Computing determinants in \ref{eq:AppleWensleydale},
	\begin{align*}
		1 & = \det g                                                  \\
